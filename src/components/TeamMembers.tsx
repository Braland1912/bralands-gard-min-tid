          ...
          {workers.map((worker) => {
            ...
            return (
              <Card key={worker.id} className="p-3 space-y-2">
                ...
                <div className="flex items-center justify-between gap-2 pt-1 border-t border-border">
                  <span className="text-xs text-muted-foreground">Får administrera checklistor</span>
                  <Switch
                    checked={(worker as any).can_manage_checklists === true}
                    onCheckedChange={async (v) => {
                      const { error } = await supabase
                        .from("workers")
                        .update({ can_manage_checklists: v } as any)
                        .eq("id", worker.id);
                      if (error) {
                        toast.error("Kunde inte uppdatera behörighet");
                        return;
